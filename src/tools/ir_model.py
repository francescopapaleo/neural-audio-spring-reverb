...
def measure_model_ir(args):
    ...
    plot_ir_spectrogram(impulse_response, config['sample_rate'], f"Model: {config['name']} Spectrogram", spectra_file)

    # Plot the waterfall spectrogram
    waterfall_file = Path(args.plots_dir) / f"IR_{Path(args.checkpoint).stem}_waterfall.png"
