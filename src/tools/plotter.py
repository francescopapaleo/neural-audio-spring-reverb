...
def plot_rt60(T, energy_db, e_5db, est_rt60, rt60_tgt, file_name, args):
    ...
    plt.vlines(est_rt60, energy_db[-1], 0, linestyles="dashed", label="Estimated RT60")
...
def plot_signals(
    ...
    plot_data(
        get_time_stamps_np(len(sweep), sample_rate),
        sweep,
        ax[0],
    ...
    plot_data(
        get_time_stamps_np(len(inverse_filter), sample_rate),
        inverse_filter,
        ax[1],
        "Inverse Filter",
        "Time [s]",
    ...
    plot_data(
        get_time_stamps_np(len(measured), sample_rate),
        measured,
        ax[2],
        "Impulse Response",
        "Time [s]",
