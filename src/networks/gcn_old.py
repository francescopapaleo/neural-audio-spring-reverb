...
class GatedConv1d(torch.nn.Module):
    ...
    def forward(self, x):
        residual = x
        y = self.conv(x)
        z = torch.tanh(y[:, : self.out_channels, :]) * torch.sigmoid(
            y[:, self.out_channels :, :]
        )
...
class GCN_FiLM(torch.nn.Module):
    def __init__(
        ...
        # output mixing layer
        self.mix = nn.Conv1d(
            in_channels=num_channels * num_layers * num_blocks,
            out_channels=1,
            kernel_size=1,
        )

    def forward(self, x, c=None):
        z_storage = torch.empty([x.shape[0], self.mix.in_channels, x.shape[2]]).to(
            x.device
        )
        film_index = 0
        z_index = 0
        ...
        for n, layer in enumerate(self.layers):
            if isinstance(layer, GatedConv1d):
                ...
                z_storage[
                    :,
                    z_index * self.num_channels : (z_index + 1) * self.num_channels,
                    :,
                ] = zn
                z_index += 1
