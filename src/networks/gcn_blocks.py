...
class GatedConv1d(nn.Module):
    def __init__(self,
        ...
        self.conv = Conv1dCausal(in_channels=in_ch,
                                    out_channels=out_ch * 2,
                                    kernel_size=kernel_size,
                                    stride=1,
                                    dilation=dilation)

        self.film = FiLM(cond_dim=cond_dim,
                         num_features=out_ch)
    ...
    def forward(self, x: torch.Tensor, c: torch.Tensor) -> Tuple[Tensor, Tensor]:
        ...
        zeros_tensor = torch.zeros(residual.size(0), self.out_ch, residual.size(2) - z.size(2), device=z.device)
...
class GCN(nn.Module):
    def __init__(self,
        ...
        for b in range(n_blocks):
            self.blocks.append(GCNBlock(in_ch=1 if b == 0 else n_channels,
                                        out_ch=n_channels,
                                        n_layers=n_layers,
                                        kernel_size=kernel_size,
        ...
        # output mixing layer
        self.blocks.append(
            torch.nn.Conv1d(in_channels=n_channels * n_layers * n_blocks,
                            out_channels=1,
                            kernel_size=1,
                            stride=1,
                            padding=0)
    ...
    def forward(self, x: torch.Tensor, c: torch.Tensor) -> Tensor:
        ...
        z = torch.empty([x.shape[0], self.blocks[-1].in_channels, x.shape[2]], device=x.device)

        for n, b in enumerate(self.blocks[:-1]):
            block = b
            x, z_n = block(x, c)
            z[:,
              n * self.n_channels * self.n_layers:
              (n + 1) * self.n_channels * self.n_layers,
              :] = z_n
