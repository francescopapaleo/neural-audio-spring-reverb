...
def load_egfx_data(datadir, batch_size):
    egfx = mirdata.initialize('orchset')  # get the orchset dataset
    egfx.download()  # download orchset
    egfx.validate()  # validate orchset
    track = egfx.choice_track()  # load a random track
    print(track)  # see what data a track contains
    egfx.track_ids()  # load all track ids
